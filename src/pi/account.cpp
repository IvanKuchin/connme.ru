...
int main()
{
	...
	*/
		...
		if(action == "AJAX_getFaq")
		{
			...
			vector<string>		keys		= {"book"};
			map<string, string> vars		= {{"itemType", "book"}, {"itemID", "1234567890"}, {"folderName", ""}, {"fileName", ""}};
			auto				config_line	= config.GetFromFile("db_update_logo_by_id", keys, vars)["book"];
			MESSAGE_DEBUG("", action, config_line);
