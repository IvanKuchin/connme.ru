...
string GetConnectionStatus()
{
	...
	while(connectionIterator)
	{
		...
		char			userNameUTF8[1024];
		char			userNameLastUTF8[1024];

		memset(date, 0, sizeof(date));
		memset(userNameUTF8, 0, sizeof(userNameUTF8));
		memset(userNameLastUTF8, 0, sizeof(userNameLastUTF8));
		...
		{
			if(!convert_cp1251_to_utf8(connectionIterator->user->GetName().c_str(), userNameUTF8, sizeof(userNameUTF8)))
			{
				{
					CLog	log(ADMIN_LOG_FILE_NAME);
					log.Write(ERROR, "GetConnectionStatus: ERROR: converting userName to UTF8");
				}
			}
			if(!convert_cp1251_to_utf8(connectionIterator->user->GetNameLast().c_str(), userNameLastUTF8, sizeof(userNameLastUTF8)))
			{
				{
					CLog	log(ADMIN_LOG_FILE_NAME);
					log.Write(ERROR, "GetConnectionStatus: ERROR: converting userNameLast to UTF8");
				}
			}
