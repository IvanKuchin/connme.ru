...
void CFile::SetContent(char *c)
{
	...
	{
		CLog	log;
		log.Write(PANIC, "void CFile::SetContent: PANIC error memory allocation. Aborting...");
...
void CFile::Delete()
{
	if(content != NULL)
	{
	...
	
		{
			...
			log.Write(DEBUG, "void CFile::Delete(): delete files from memory");
		}
...
bool CFiles::Add(string name, char *content, int size)
{
	...
	if(!f)
	{
		...
		log.Write(PANIC, "bool CFiles::Add(string name, char *content, int size): error allocating memory (file module)");
